...
export function ImageUpload({
  ...
  return <div className={cn("space-y-4", className)}>
      ...
      <div onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop} className="rounded-sm">
        <div className="flex flex-col items-center justify-center text-center rounded-xl">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Button disabled={disabled} onClick={() => document.getElementById("file-upload")?.click()} size="lg" className="px-8 py-6 text-lg h-auto">
              <FileUp className="mr-2" />
              Upload Documents
            </Button>
            <input id="file-upload" type="file" multiple accept={acceptedFileTypes.join(",")} className="hidden" onChange={handleChange} disabled={disabled} />
          </div>
        </div>
      </div>
      ...
      {/* Preview */}
      {value.length > 0 && <div className="mt-8 glass-morphism rounded-xl p-4 animate-scale-in">
          <h3 className="text-lg font-medium mb-4">Uploaded Documents</h3>
          <div className="space-y-3 max-h-72 overflow-y-auto minimal-scrollbar pr-2">
            {value.map((fileWithPreview, index) => <div key={index} className="flex items-center p-3 bg-black/30 rounded-lg shadow-sm border border-white/5">
                <div className="relative h-14 w-14 rounded-md bg-muted mr-3 overflow-hidden">
                  {fileWithPreview.file.type.startsWith("image/") ? <img src={fileWithPreview.preview} alt={fileWithPreview.file.name} className="h-full w-full object-cover" /> : <div className="h-full w-full flex items-center justify-center bg-primary/10">
                ...
                <button type="button" onClick={() => handleRemove(index)} className="p-1 hover:bg-white/10 rounded-full transition-colors" disabled={disabled}>
